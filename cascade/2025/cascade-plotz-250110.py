...
if 1: # PTFE, cold Xe gas, alphas from cathode, PMT bottom
	data_folders = np.array(['20241219-121719'])
	colorz = np.array(['gray','steelblue','olivedrab','goldenrod','firebrick','sienna'])
	labl = np.array(['-88C gas','','',''])
...
for ii in range(0,1):#data_folders.shape[0]):
	...
	aa = np.zeros([17,h_n_events])
	ee = np.zeros([17,h_n_events,20])
	s1 = np.zeros([17,h_n_events])
	is1 = np.zeros([h_n_events])
	is2 = np.zeros([h_n_events])
	s2cf = np.zeros([h_n_events])
	ss = np.zeros([3,h_n_events])
	aa_last = 0
	...
	#	np.savez(aa_file,aa,ee,s1,is1,is2,s2f)
		with np.load(aa_file) as data:
			...
			ss[:,aa_last:(aa_last+a)] = data["arr_6"]
		aa_last = aa_last + a
	...
	s1bot = np.sum(s1[16:17,np.arange(0,ei,4)],axis=0)
...
# 	cut = (s10>500) & (s10<1500)
	...
	pl.figure(6);pl.clf();
	db=0.3
	beans = np.arange(0,30,db)
	beanc = (beans[1:]+beans[0:-1])/2
	[cts,beans] = np.histogram(ss[:,np.arange(0,ei,4)],beans); cts[0]=0 
	pl.errorbar(beanc,cts,yerr=np.sqrt(cts),xerr=db/2,fmt='k.')
	[cts,beans] = np.histogram(ss[:,np.arange(1,ei,4)],beans); cts[0]=0 
	pl.errorbar(beanc,cts,yerr=np.sqrt(cts),xerr=db/2,fmt='r.')
	[cts,beans] = np.histogram(ss[:,np.arange(2,ei,4)],beans); cts[0]=0 
