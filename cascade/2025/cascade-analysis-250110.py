...
plotz=1
if 1: # 25A single PMT bottom, cathode alphas (?)
	data_dir = '/Users/peter/Public/data/20250114-210405/'
...
if 0: # original setup with 16 sipms on board 0 (ch 1 - ch 16) and a pmt on board 1 (ch 17)
	...
	nch = sipms.shape[0]
	spe_sizes = np.loadtxt(spe_dir, dtype='float') # mV ns
	spe_sizes[15] = spe_sizes[16]; spe_sizes = spe_sizes[0:16]
	pmt_ch = 15
...
for compressed_file in compressed_file_list:
	if 1:
		...
		ch_data_mV[pmt_ch,:,:] = ch_data_mV[pmt_ch,:,:]/ds_pmt # downsize the PMT to make it SiPM size
	...
	if 0:
		...
		a = np.zeros(4)
		for i in range (0,3):
			a[i] = np.sum((ch_data_mV[:,i,:]>0.5) * ch_data_mV[:,i,:])
	n_start = int(np.argmax(a))
...
# 				pl.plot(t[25000:],2+y0p5z[i,:],'-',linewidth=0.5)
			pl.plot(t,1+y0z[pmt_ch,:],'k-',linewidth=0.5)
...
# 			pl.xlim([4,20])
			...
			try:
				...
				print('')
			pl.plot(t,1+y1z[pmt_ch,:],'k-',linewidth=0.5);
			...
			try:
				...
				print('')
			pl.plot(t,1+y2z[pmt_ch,:],'k-',linewidth=0.5);
			...
			try:
				...
				print('')
			pl.plot(t,1+y3z[pmt_ch,:],'k-',linewidth=0.5);
